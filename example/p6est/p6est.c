...
p6est_destroy (p6est_t * p6est)
{
  ...
  sc_allgather_final_destroy(p6est->global_first_layer, p6est->mpicomm);
  P4EST_FREE (p6est);
}
...
p6est_update_offsets (p6est_t * p6est)
{
  ...
  if (gfl != NULL) {
    sc_allgather_final_destroy (gfl, p6est->mpicomm);
    gfl = NULL;
  }
  sc_allgather_final_scan_create (&mycount, (void **) &gfl,
                                  1, P4EST_MPI_GLOIDX, sc_MPI_SUM,
                                  p6est->mpicomm);
  P4EST_ASSERT ((size_t) (gfl[p6est->mpirank + 1] - gfl[p6est->mpirank]) ==
                p6est->layers->elem_count);
  p6est->global_first_layer = gfl;
}
                     ...
                     p6est_weight_t weight_fn)
{
  ...
  new_gfl = NULL;
  sc_allgather_final_scan_create(&my_count, (void **) &new_gfl, 1,
                                 P4EST_MPI_GLOIDX, sc_MPI_SUM, p6est->mpicomm);
  ...
  if (old_gfl[rank] == new_gfl[rank] &&
    ...
    sc_allgather_final_destroy(old_gfl,p6est->mpicomm);
